...
class ImageGenerator(object):
    def __init__(self, x, y):
        ...
        self.y = y
        self.N = x.shape[0]
        self.n_sample, self.height, self.width, self.n_chanel = x.shape
        
        self.trans_height = 0
        self.trans_width  = 0
        self.horiz_flip  = False
        self.verti_flip  = False
    ...
    def next_batch_gen(self, batch_size, shuffle=True):
        ...
        n_sample = x_aug.shape[0]
        n_batch  = n_sample//batch_size
        batch_count = 0
        
        while True:
            if batch_count < n_batch:
                select_range = np.arange(batch_count*batch_size,(batch_count+1)*batch_size)
                yield (x_aug[select_range,:,:,:],y_aug[select_range])
                batch_count += 1
            else:
                if shuffle:
                    index = np.random.choice(n_sample,n_sample,replace=False)
                    x_aug,y_aug = x_aug[index], y_aug[index]
                self.x_aug = x_aug
                self.y_aug = y_aug
                
                # rest the batch_count
                batch_count = 0
    ...
    def flip(self, mode='h'):
        ...
        if mode == 'h':
            ...
            self.horiz_flip = not self.horiz_flip
        elif mode == 'v':
            flipped = np.flip(self.x.copy(), axis=1)
            self.verti_flip = not self.verti_flip
        elif mode == 'hv':
        ...
        print('Vertical flip: ', self.verti_flip, 'Horizontal flip: ', self.horiz_flip)
    ...
    def add_noise(self, portion, amplitude):
        ...
        select_index = np.random.choice(x_copy.shape[0],size = int(portion*x_copy.shape[0]), replace = False)
        
        for i in select_index:
            
            x_copy[i] += int(amplitude*np.random.rand(1))
        ...
        print('Current noise level is: ', portion, amplitude)
        return x_copy
        #######################################################################
        #                                END TODO                             #
        #######################################################################
