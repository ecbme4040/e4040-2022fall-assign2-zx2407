...
def conv2d_forward(x, w, b, pad, stride):
    ...
    n_batch, height, width, n_channel = x.shape
    filter_height, filter_width, n_channel, n_filter = w.shape
    
    pad_input = np.zeros([n_batch, height+2*pad, width+2*pad, n_channel])
    pad_input[:,pad:pad+height,pad:pad+width,:] = x
    
    new_height = ((height - filter_height + 2 * pad) // stride) + 1
    new_width  = ((width  - filter_width  + 2 * pad) // stride) + 1
    
    output     = np.zeros([n_batch, new_height, new_width, n_filter])
    
    for n in range(n_batch):
        for i in range(new_height):
            for j in range(new_width):
                for k in range(n_filter):
                    output[n,i,j,k] = np.sum(w[:,:,:,k]*pad_input[n, (i*stride):(i*stride+filter_height), (j*stride):(j*stride+filter_width), :]) + b[k]
...
def avg_pool_forward(x, pool_size, stride):
    ...
    #######################################################################
    # raise NotImplementedError
    n_batch, height, width, n_filters = x.shape
    new_height, new_width = height//stride, height//stride
    
    output = np.zeros([n_batch, new_height, new_width, n_filters])
    
    for n in range(n_batch):
        for i in range(new_height):
            for j in range(new_width):
                for k in range(n_filters):
                    output[n, i, j, k] = np.mean(x[n, (i*stride):(i*stride+pool_size), (j*stride):(j*stride+pool_size), k])
...
def avg_pool_backward(dout, x, pool_size, stride):
    ...
    # input dimensions
    n_channel = x.shape[-1]
    n_batch, height_new, width_new, n_filters = dout.shape
        ...
        for i in range(height_new):
            for j in range(width_new):
                for k in range(n_channel):
                    ...
                    average_dA=dout[n,i,j,k]/2/2
                    dx[n,(i*stride):(i*stride+pool_size),(j*stride):(j*stride+pool_size),k] += np.ones((height_new, width_new)) * average_dA
