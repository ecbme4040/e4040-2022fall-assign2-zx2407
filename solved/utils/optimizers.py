...
class SGDmomentumOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            velocities[k] = velocities[k] * momentum + learning_rate * grads[k]
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            m = momentums[k]/(1-pow(beta1,t))
            velocities[k] = beta2 * velocities[k] + (1-beta2) *pow(grads[k],2)
            v = velocities[k]/(1-pow(beta2,t))
            params[k] -= (learning_rate/(pow(v,0.5)+eps)) * m
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            velocities[k] = beta2 * velocities[k] + (1-beta2) *pow(grads[k],2)
            v = velocities[k]/(1-pow(beta2,t))
            params[k] -= (learning_rate/(pow(v,0.5)+eps)) * (beta1 * m + ((1 - beta1)*grads[k])/(1-pow(beta1,t)))
        ###################################################
        #               END OF YOUR CODE                  #
        ###################################################
